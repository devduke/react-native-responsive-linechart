...
exports.formatDataForSVG = function (data) {
    return data.map(function (p) { return p.x + ',' + p.y; }).join(' ');
};
exports.scalePointToDimensions = function (data, domain, dimensions) { return ({
    x: scaleXValueToDimensions(data.x, domain, dimensions),
    y: scaleYValueToDimensions(data.y, domain, dimensions),
}); };
exports.scalePointsToDimensions = function (data, domain, dimensions) {
...
exports.appendPointsToPath = function (path, points) {
    return path + " " + points.map(function (p) { return "L " + p.x + " " + p.y; }).join(' ');
};
exports.svgPath = function (points, smoothing, tension) {
    if (smoothing === 'bezier') {
