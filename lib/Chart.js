...
var Chart = React.memo(function (props) {
    ...
    var _onTouchGestureEvent = react_native_1.Animated.event([{ nativeEvent: {} }], {
        ...
        listener: function (evt) {
            ...
            if (evt.nativeEvent.state === react_native_gesture_handler_1.State.BEGAN) {
                handleTouchEvent(evt.nativeEvent.x, evt.nativeEvent.y);
            }
            if (evt.nativeEvent.state === react_native_gesture_handler_1.State.ACTIVE || evt.nativeEvent.state === react_native_gesture_handler_1.State.END) {
                setLastTouch({
                    type: 'touchEnd',
                });
            }
